...
function MovieDetails(props){
  ...
  const rateClicked = rate => evt => {
    fetch(`https://rating-application.herokuapp.com/api/movies/${mov.id}/rate_movie/`, {
      method:'POST',
      headers:{
  ...
  const getDetails = () => {
    fetch(`https://rating-application.herokuapp.com/api/movies/${mov.id}/`, {
      method:'GET',
      headers:{
        'Content-Type':'application/json',
        'Authorization': `Token ${token['mr-token']}`
      ...
      {mov? (
        <div>
          ...
          <div className="rate-container">
            ...
            {
              [...Array(5)].map( (e, i) => {
                return <FontAwesomeIcon key={i} icon={faStar} className={highlighted>i-1?'purple':''} onMouseEnter={highlightRate(i)} onMouseLeave={highlightRate(-1)} onClick={rateClicked(i)}/>
              })
            }
          </div>
        </div>
